...
task("startIncomeSharingPool", "Start a new income sharing pool")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const vault: IncomeSharingVault = new IncomeSharingVault__factory(dev_account).attach(vaultAddress);
...
task("setIncomeSpeed", "Set income sharing speed")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const tx = await vault.setRewardSpeed(taskArgs.pid, parseUnits(taskArgs.reward, 6));

    console.log("Tx details: ", await tx.wait());
  });

task("setRoundTime", "Set income sharing round time").setAction(async (taskArgs, hre) => {
  const { network } = hre;
...
task("getIncomeBalance", "Get balance in income sharing vault").setAction(async (taskArgs, hre) => {
  const { network } = hre;
  ...
  const usd = new MockUSD__factory(dev_account).attach(getTokenAddressOnAVAX("USDC.e"));

  const balance = await usd.balanceOf(vaultAddress);
  console.log("Balance: ", formatUnits(balance, 6));

  const emergencyBalance = await usd.balanceOf(addressList[network.name].EmergencyPool);
...
task("updateLastRewardBalance", "Update last reward balance").setAction(async (_, hre) => {
  const { network } = hre;
  ...
  const vault: IncomeSharingVaultV2 = new IncomeSharingVaultV2__factory(dev_account).attach(vaultAddress);
