...
task("setIncomeMakerInFactory", "Set the income maker address inside naughty factory").setAction(async (_, hre) => {
  ...
  const factory: NaughtyFactory = new NaughtyFactory__factory(dev_account).attach(factoryAddress);
  ...
  console.log("The income maker address inside naughty factory: ", makerAddress);
});

task("setIncomeSharingInCore", "Set the income sharing contract address inside policy core").setAction(
  async (_, hre) => {
    const { network } = hre;
    ...
    const core: PolicyCore = new PolicyCore__factory(dev_account).attach(policyCoreAddress);

    const tx_setIncomeSharing = await core.setIncomeSharing(incomeSharingAddress);
    console.log("Tx_setIncomeSharing details: ", await tx_setIncomeSharing.wait());
...
task("addIncomeSharingWL", "Add income sharing contract to veDEG whitelist").setAction(async (_, hre) => {
  ...
  const veDEG: VoteEscrowedDegis = new VoteEscrowedDegis__factory(dev_account).attach(veDEGAddress);
