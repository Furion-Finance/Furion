...
describe("Furion Farming Pools", function(){
    ...
    describe("Owner Functions", function () {
        it("should be able to set the start block", async function () {
            await expect(farmingPool.setStartTimestamp(60000))
                .to.emit(farmingPool, "StartTimestampChanged")
                .withArgs(60000);
        });
        ...
        it("should not be able to set start block after start mining", async function () {
            ...
            await expect(farmingPool.setStartTimestamp(60000)).to.be.revertedWith(
                "ALREADY_HAVING_POOLS"
            );
        });

        it("should be able to pause the contract", async function () {
            await expect(farmingPool.connect(user1).pause()).to.be.revertedWith(
                "Ownable: caller is not the owner"
            );
            await expect(farmingPool.pause())
                .to.emit(farmingPool, "Paused")
                .withArgs(dev.address);

            await expect(
                farmingPool.add(pairAddress, toWei("5"), false)
        ...
        it("should be able to unpause the contract", async function () {
            ...
            await expect(farmingPool.unpause())
                .to.emit(farmingPool, "Unpaused")
                .withArgs(dev.address);
        ...
        it("should be able to add new pools", async function () {
            ...
            expect(await farmingPool.poolMapping(pairAddress)).to.equal(
                nextPoolId.sub(2)
            );
            expect(await farmingPool.poolMapping(erc.address)).to.equal(
                nextPoolId.sub(1)
            );
        ...
        it("should not be able to add two same pools", async function () {
            ...
            await expect(
                farmingPool.add(pairAddress, toWei("5"), false)
            ).to.be.revertedWith("FARMING_POOL: ALREADY_IN_POOL");
        });
        ...
        it("should be able to set Furion reward of a pool", async function () {
            ...
            const poolList = await farmingPool.getPoolList();
            expect(poolList[poolId.toNumber()].basicFurionPerSecond).to.equal(
                toWei("10")
            );
        });
    ...
    describe("User Functions: Deposit, Redeem and Harvest", function () {
        ...
        it("should be able to get correct rewards after another user deposit", async function () {
            ...
            expect(await farmingPool.pendingFurion(2, user1.address)).to.equal(
                toWei("30")
            );
            ...
            // 1 * 10 * 4 / (4 + 1)
            expect(await farmingPool.pendingFurion(2, dev.address)).to.equal(
                toWei("8")
            );
