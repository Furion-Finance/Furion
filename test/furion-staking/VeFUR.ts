...
describe("Vote Escrowed Furion", function () {
    ...
    describe("Owner Functions", function () {
        ...
        it("should be able to change the max cap ratio", async function () {
            await expect(veFUR.setMaxCapRatio(50))
                .to.emit(veFUR, "MaxCapRatioChanged")
                .withArgs(100, 50);
        });
        it("should be able to add and remove whitelist address", async function () {
            await expect(veFUR.addWhitelist(user1.address))
                .to.emit(veFUR, "WhiteListAdded")
                .withArgs(user1.address);

            await expect(veFUR.removeWhitelist(user1.address))
                .to.emit(veFUR, "WhiteListRemoVEF")
                .withArgs(user1.address);
        });
    });
    ...
    describe("Deposit and Claim", function () {
        ...
        it("should be able to deposit FUR tokens", async function () {
            ...
            await mineBlocks(10);
            expect(await veFUR.claimable(dev.address)).to.equal(
                toWei("1000")
            );
        ...
        it("should be able to claim the veFUR reward", async function () {
            ...
            expect(await veFUR.balanceOf(dev.address)).to.equal(
                toWei("1000")
            );
        ...
        it("should be able to withdraw FUR tokens", async function () {
            ...
            expect(await furion.balanceOf(dev.address)).to.equal(
                toWei("1000")
            );
        });
    ...
    describe("Whitelist contract lock veFUR", function () {
        ...
        it("should not be able to lock & unlock veFUR by accounts other than whitelist", async function () {
            ...
            // Owner can not lock
            await expect(
                veFUR.lockVeFUR(dev.address, toWei("100"))
            ).to.be.revertedWith(customErrorMsg("'VEF__NotWhiteListed()'"));

            await expect(
                veFUR.unlockVeFUR(dev.address, toWei("100"))
            ).to.be.revertedWith(customErrorMsg("'VEF__NotWhiteListed()'"));

            // Not whitelisted accounts can not lock
            await expect(
                veFUR.connect(user2).lockVeFUR(dev.address, toWei("100"))
            ).to.be.revertedWith(customErrorMsg("'VEF__NotWhiteListed()'"));

            await expect(
                veFUR.connect(user2).unlockVeFUR(dev.address, toWei("100"))
            ).to.be.revertedWith(customErrorMsg("'VEF__NotWhiteListed()'"));
        });
        ...
        it("should be able to lock and unlock veFUR", async function () {
            ...
            await expect(
                veFUR.connect(user1).lockVeFUR(dev.address, toWei("100"))
            )
            ...
            await expect(veFUR.withdraw(toWei("100"))).to.be.revertedWith(
                customErrorMsg("'VEF__StillLocked()'")
            );

            await expect(
                veFUR.connect(user1).unlockVeFUR(dev.address, toWei("100"))
            )
