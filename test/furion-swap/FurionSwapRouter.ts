...
import { formatStablecoin, formatTokenAmount, getLatestBlockTimestamp, stablecoinToWei, toBN, toWei } from "../utils";
...
describe("Furion Swap V2 Router", function () {
  ...
  describe("Add and remove liquidity for tokens(not ETH)", function () {
    it("should be able to add liquidity to mint LP token", async function () {
      ...
      await expect(
        ...
        .withArgs(pairAddress1, toWei("100"), stablecoinToWei("100"), stablecoinToWei("99999999.999"));
    ...
    it("should be able to remove liquidity from the pool", async function () {
      ...
      await expect(
        ...
        .withArgs(pairAddress1, balance0, balance1, stablecoinToWei("99999999.999"));
    });
  });
  ...
  describe("Add and remove liquidity for ETH", function () {
    it("should be able to add liquidity to mint LP token", async function () {
      ...
      await expect(
        router.addLiquidityETH(erc.address, toWei("100"), toWei("80"), toWei("80"), dev.address, now + txDelay, {
          value: toWei("100"),
        }),
      )
        .to.emit(router, "LiquidityAdded")
        .withArgs(pairAddress2, toWei("100"), toWei("100"), toWei("99.999999999999999"));
    ...
    it("should be able to remove liquidity from the pool", async function () {
      ...
      await router.addLiquidityETH(erc.address, toWei("100"), toWei("80"), toWei("80"), dev.address, now + txDelay, {
        value: toWei("100"),
      });
      ...
      await expect(
        ...
        .to.emit(router, "LiquidityRemoved")
        .withArgs(pairAddress2, toWei("99.999999999999999"), toWei("99.999999999999999"), toWei("99.999999999999999"));
    });
  });
  ...
  describe("Swap tokens with ETH or for ETH", function () {
    beforeEach(async function () {
      ...
      await router.addLiquidityETH(erc.address, toWei("1000"), toWei("80"), toWei("80"), dev.address, now + txDelay, {
        value: toWei("1000"),
      });
    });
    ...
    it("should be able to swap tokens for exact ETH", async function () {
      ...
      await expect(
        router
          .connect(user1)
          .swapTokensForExactETH(toWei("10"), toWei("1"), [erc.address, usd.address], user1.address, now + txDelay),
      ).to.revertedWith("FurionSwapV2Router: INVALID_PATH");

      await expect(
        router
          .connect(user1)
          .swapTokensForExactETH(toWei("10"), toWei("1"), [erc.address, weth.address], user1.address, now + txDelay),
      ).to.revertedWith("FurionSwapV2Router: EXCESSIVE_INPUT_AMOUNT");

      await router
        .connect(user1)
        .swapTokensForExactETH(toWei("10"), toWei("1000"), [erc.address, weth.address], user1.address, now + txDelay);
    ...
    it("should be able to swap exact tokens for ETH", async function () {
      ...
      await expect(
        router
          .connect(user1)
          .swapExactTokensForETH(toWei("100"), toWei("1"), [erc.address, usd.address], user1.address, now + txDelay),
      ).to.revertedWith("FurionSwapV2Router: INVALID_PATH");

      await expect(
        router
          .connect(user1)
          .swapExactTokensForETH(toWei("100"), toWei("100"), [erc.address, weth.address], user1.address, now + txDelay),
      ).to.revertedWith("FurionSwapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");

      await router
        .connect(user1)
        .swapExactTokensForETH(toWei("100"), toWei("1"), [erc.address, weth.address], user1.address, now + txDelay);
    ...
    it("should be able to swap ETH for exact tokens", async function () {
      ...
      await expect(
        router
          .connect(user1)
          .swapETHForExactTokens(toWei("10"), [usd.address, weth.address], user1.address, now + txDelay, {
            value: toWei("10"),
          }),
      ).to.revertedWith("FurionSwapV2Router: INVALID_PATH");
      ...
      await expect(
        router
          .connect(user1)
          .swapETHForExactTokens(toWei("10"), [weth.address, erc.address], user1.address, now + txDelay, {
            value: toWei("10"),
          }),
      ).to.revertedWith("FurionSwapV2Router: EXCESSIVE_INPUT_AMOUNT");
    ...
    it("should be able to swap exact ETH for tokens", async function () {
      ...
      await expect(
        router
          .connect(user1)
          .swapExactETHForTokens(toWei("10"), [usd.address, erc.address], user1.address, now + txDelay, {
            value: toWei("10"),
          }),
      ).to.revertedWith("FurionSwapV2Router: INVALID_PATH");
      ...
      await expect(
        router
          .connect(user1)
          .swapExactETHForTokens(toWei("10"), [weth.address, erc.address], user1.address, now + txDelay, {
            value: toWei("10"),
          }),
      ).to.revertedWith("FurionSwapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
      ...
      await router
        .connect(user1)
        .swapExactETHForTokens(toWei("1"), [weth.address, erc.address], user1.address, now + txDelay, {
          value: toWei("10"),
        });
